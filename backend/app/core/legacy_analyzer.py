...
class LegacyAnalyzer:
    ...
    async def export_knowledge_summary(
        ...
        """
        Export a human-readable summary of extracted knowledge from parsed results
        """
        ...
        summary_content = f"""# PowerBuilder Knowledge Extraction Summary

**Database**: {database_sid}
**Schema**: {schema_name}
**Date**: {Path(output_path).stat().st_mtime if Path(output_path).exists() else "Just now"}

## 1. Tables Discovered ({len(tables)})
{', '.join(sorted(list(tables)))}

## 2. SQL Query Patterns ({len(sql_queries)})
Extracted {len(sql_queries)} query patterns from source code. 
These are used to understand table JOIN relationships and column usage patterns.

## 3. Business Rules ({len(business_rules)})
{chr(10).join([f"- {rule.get('rule', 'N/A')} (Source: {rule.get('source_file', 'N/A')})" for rule in business_rules[:20]])}
{f"... and {len(business_rules) - 20} more" if len(business_rules) > 20 else ""}

## 4. Complexity Analysis
{json.dumps(self.analyze_legacy_complexity(parsed_results), indent=2)}

---
*Generated by Oracle NL-SQL Legacy Analyzer*
"""
        # Save to file
